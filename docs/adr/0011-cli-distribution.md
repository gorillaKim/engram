...
1. **1차 (즉시 지원): `cargo install`**.
   - 정식 명령: `cargo install --git https://github.com/gorillaKim/engram engram-cli` (workspace 안의 `engram-cli` 크레이트 명).
...
2. **2차 (Release 워크플로 도입): GitHub Releases prebuilt binary**.
   ...
   - 사용자는 `curl -L https://github.com/gorillaKim/engram/releases/download/v<ver>/engram-<ver>-aarch64-apple-darwin.tar.gz | tar xz` 로 단일 바이너리 추출 후 `$PATH` 에 배치.
