...
- [x] Tauri 앱 기동 + 메인 윈도우에 칸반 5컬럼 표시 (KanbanBoard.tsx — 5컬럼 그리드 구현)
- [x] CLI 로 만든 데이터가 5초 내 자동 반영 (useBoardStatus `refetchInterval: 5000`)
- [x] single-instance 플러그인 동작 (`tauri_plugin_single_instance::init` 등록)
- [x] `pnpm build` 및 `cargo build -p engram-desktop` clean (`cargo test --workspace`: 41 passed)
