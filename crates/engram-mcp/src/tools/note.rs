...
pub fn tool_definitions() -> Vec<Value> {
    vec![
        json!({ "name": "note_add",
            "description": "이슈/태스크에 구조화된 노트를 추가합니다. note_type: caveat(주의)/decision(결정)/discovery(발견)/blocker_detail(블로커)/context(인수인계)/reference(참조)/comment(사용자-에이전트 대화). author 는 기본 'agent', 에이전트가 사용자 코멘트에 답변할 때도 'agent' 로 남기세요.",
            "inputSchema": { "type": "object", "required": ["issue_id", "note_type", "summary"],
                "properties": {
                    ...
                    "author":    { "type": "string", "description": "작성자 식별자. 기본 'agent'. 사용자 작성은 'user'." }
                }
            }
        }),
        json!({ "name": "note_list",
