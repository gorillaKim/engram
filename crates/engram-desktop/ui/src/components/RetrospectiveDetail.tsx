...
export function RetrospectiveDetail({
  ...
  const pendingCount = retro.action_items.filter((a) => a.status !== 'done').length;
  ...
  return (
    <div className="flex flex-col h-full bg-white text-slate-900 shadow-2xl min-w-0">
        ...
        <div className="flex items-center gap-3 shrink-0">
          {pendingCount > 0 && (
            <button
              ...
              className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-xs shadow-sm transition-all active:scale-95 whitespace-nowrap shrink-0"
            >
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-4 p-6 overflow-y-auto bg-slate-50/50 min-w-0">
        ...
        {/* 우측 1열: Action Items 패널 (기존 이슈 히스토리 연결 기능 지원) */}
        <div className="flex flex-col gap-4 bg-white rounded-xl p-4 border border-slate-200 shadow-sm min-w-0">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3 shrink-0">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <span className="px-2 py-0.5 rounded-full text-[11px] bg-slate-100 text-slate-600 shrink-0">
          ...
          <form onSubmit={handleAdd} className="flex gap-2 shrink-0">
            <input
              ...
              className="flex-1 px-3 py-2 text-xs bg-slate-50 border border-slate-200 rounded-lg text-slate-900 placeholder-slate-400 focus:outline-none focus:border-indigo-500 min-w-0"
            />
            <button
              type="submit"
              className="px-3.5 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold flex items-center gap-1 border border-slate-200 shrink-0 whitespace-nowrap"
            >
                  ...
                  <div className="flex items-center justify-end gap-1.5 pt-1.5 border-t border-slate-200/60 shrink-0">
                    {item.linked_issue_id ? (
                      <button
                        ...
                        className="flex items-center gap-1 px-2.5 py-1 rounded-md bg-indigo-50 text-indigo-700 border border-indigo-200 text-[11px] font-mono hover:bg-indigo-100 transition-colors shrink-0 whitespace-nowrap"
                      ...
                      <>
                        <button
                          ...
                          className="flex items-center gap-1 px-2 py-1 rounded bg-white hover:bg-slate-100 text-slate-600 border border-slate-200 text-[11px] transition-all shrink-0 whitespace-nowrap shadow-2xs"
                        ...
                        <button
                          ...
                          className="flex items-center gap-1 px-2 py-1 rounded bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200 text-[11px] transition-all shrink-0 whitespace-nowrap shadow-2xs font-medium"
